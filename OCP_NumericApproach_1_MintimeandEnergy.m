...
%% objective function 
% S = 1e-4; % weight matrix to act on time
% R = 1e-02 * eye(2); % weight matrix to act on input torque
% 
...
for i = 1:N
    ...
    J = J + u(:, i)' * R * u(:, i) + (u(:, i) - u(:, i-1))' * R_ * (u(:, i) - u(:, i-1)) ; %through that ,a compromise bet minimal time  and minimal control energy
end

opti.minimize(J + tendopt);
