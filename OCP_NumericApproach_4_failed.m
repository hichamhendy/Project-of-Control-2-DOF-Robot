...
t_f = 1;
t_0 = 0;
N = 30; % % number of control intervals % Time horizon %horzions (optimizied)
intervall_time = (t_f - t_0) / N; % Duration of intervalls
...
for i = 1:N
    ...
    x_next = implicitTrapezoidal(x(:, i), u(:, i), intervall_time, tendopt);
    opti.subject_to(x(:,i+1) == x_next);
    t = [t, t(end) + intervall_time];
end
...
function x_next = implicitTrapezoidal( x, u, h ,t_opt)
% % Trapezoidal implicit integrator
k1 = stateSpace_ODE( x, u, t_opt);
k2 = stateSpace_ODE( x + k1 * h/2 + k2 * h/2 , u ,  t_opt + h  ); % defne k2 !!!!
% % Trapezoidal 2 steps per interval
x_next = x + ( k1 + k2 ) * h / 2;
